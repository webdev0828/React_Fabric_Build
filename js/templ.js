    ...
    /** @lends fabric.CurvedText.prototype */ {
      ...
      _render: function(ctx) {
        ...
        if (this.letters) {
          ...
          } else if (this.effect === 'vertical') {
            fixedLetterAngle =
              (this.letters.item(0).fontSize + space) /
              this.radius /
              (Math.PI / 180)
            textWidth = this.letters.item(0).fontSize + space
          } else if (this.effect === 'flag') {
            fixedLetterAngle =
              (this.letters.item(0).fontSize + space) /
              this.radius /
              (Math.PI / 180)
          ...
          if (this.effect === 'flag') curAngle /= 2
          ...
          for (var i = 0, len = this.text.length; i < len; i++) {
            ...
            if (this.effect === 'curved') {
              ...
              this.letters
                ...
                .set(
                  'left',
                  multiplier * (Math.sin(angleRadians) * this.radius),
                )
              this.letters.item(i).set('padding', 0)
              ...
              this.letters.item(i).set('selectable', false)
              if (i === len / 2)
                this.letters.item(i).set('transformMatrix', [1, 0, 0, 1, 0, 0])
              else if (i < len / 2)
                this.letters
                  .item(i)
                  .set('transformMatrix', [1, -0.2, 0, 1, 0, 0])
              else
                this.letters
                  .item(i)
                  .set('transformMatrix', [1, 0.2, 0, 1, 0, 0])
              ...
              //vertical
              let letter = this.letters.item(i).text
              if (
                letter === 'g' ||
                letter === 'j' ||
                letter === 'p' ||
                ...
                letter === 'y' ||
                (i >= 1 &&
                  this.letters.item(i - 1).text ===
                    this.letters.item(i - 1).text.toUpperCase() &&
                  this.letters.item(i).text ===
                    this.letters.item(i).text.toLowerCase())
              )
                this.letters
                  .item(i)
                  .set(
                    ...
                    width +
                      customSpacing -
                      this.letters.item(i).get('width') / 2,
                  )
              else this.letters.item(i).set('top', width + customSpacing)
              this.letters
                .item(i)
                .set('left', -this.letters.item(i).get('width') / 2)
              this.letters.item(i).setAngle(0)
          ...
          if (this.effect === 'flag') {
            const flag_len = Math.floor(this.text.length / 2)
            const fix_top =
              multiplier *
              -1 *
              (Math.cos(curAngle * (Math.PI / 180)) * this.radius)
            for (var i = 0; i < flag_len; i++) {
              if (i === 0) curAngle = multiplier * (multiplier * curAngle)
              else
                curAngle =
                  multiplier * (multiplier * curAngle + fixedLetterAngle)
              angleRadians = curAngle * (Math.PI / 180)
              ...
              if (
                i === Math.round(flag_len / 2) - 1 ||
                i === Math.round(flag_len / 2)
              )
                this.letters.item(i).set('transformMatrix', [1, 0, 0, 1, 0, 0])
              else if (i < flag_len / 2)
                this.letters
                  .item(i)
                  .set('transformMatrix', [1, -0.2, 0, 1, 0, 0])
              else
                ...
                this.letters
                  ...
                  .set(
                    ...
                    multiplier *
                      (Math.cos(
                        multiplier *
                          (multiplier * curAngle + 2 * fixedLetterAngle) *
                          (Math.PI / 180),
                      ) *
                        this.radius) +
                      2 * fix_top,
              ...
              this.letters.item(i + flag_len).set('padding', 0)
              this.letters.item(i + flag_len).set('selectable', false)
            ...
            this.letters
              ...
              .set(
                ...
                textWidth / 2 -
                  2 * space +
                  multiplier *
                    (Math.sin((curAngle - fixedLetterAngle) * (Math.PI / 180)) *
                      this.radius),
              )
            this.letters.item(this.text.length - 1).set('padding', 0)
            this.letters.item(this.text.length - 1).set('selectable', false)
      ...
      _renderOld: function(ctx) {
        if (this.letters) {
          ...
          for (var i = 0, len = this.text.length; i < len; i++) {
            ...
            this.letters
              .item(i)
              .set('top', multiplier - Math.cos(angleRadians) * this.radius)
            this.letters
              .item(i)
