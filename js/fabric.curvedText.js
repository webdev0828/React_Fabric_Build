        ...
        if (this.letters) {
          ...
          for (var i = 0, len = this.text.length; i < len; i++) {
            ...
            if (this.effect === "curved") {
              ...
              this.letters.item(i).set("left", width + i * space);
              this.letters.item(i).set("top", 0);
              this.letters.item(i).setAngle(0);
              width += this.letters.item(i).get("width");
              this.letters.item(i).set("padding", 0);
          ...
          if (this.effect === "flag") {
            const fixed_angle = (3 * Math.PI) / this.text.length;
            const fixed_width = textWidth / this.text.length;

            const fixed_radius = this.radius / this.text.length / 2;
            for (let i = 0; i < this.text.length; i++) {
              const angle = (i + 1) * fixed_angle;
              this.letters.item(i).set("top", Math.cos(angle) * fixed_radius);
              this.letters.item(i).set("left", i * fixed_width);
              this.letters.item(i).set("padding", 0);
              this.letters.item(i).set("selectable", false);
              if (i < this.text.length * 0.25 || i > this.text.length * 0.75) {
                this.letters
                  .item(i)
                  .set("transformMatrix", [1, -0.2, 0, 1, 0, 0]);
              } else {
